  ...
  //Function to test the range finder, onboard LED should blink slower the closer the object.
  void testRangeFinder(int range){
    int timing = (range/10)*1000;
  ...
  void makeFollowPattern(int middle){
    ...
    for(int i = middle - tailLength-EXTRA_OFF; i < middle + tailLength+EXTRA_OFF; i++){
      ...
      if(i == middle){
        leds[i] = HeatedColor(scale8((uint8_t)(i-middle), 255));
        continue;
      }
